...
@zeus What do you think?   (or /consensflow:cf @zeus What do you think?)
  ...
  -> runner launches the configured backend with its tools policy
     (default review mode unless the participant was configured write-capable)
...
- explicit tool policy (configured; missing policy means default review mode)
...
| `codex` | `codex exec --json --ephemeral --skip-git-repo-check --ignore-user-config --ignore-rules` plus `model_reasoning_effort` from preset | Uses sandbox mode for default review runs and avoids user config/rules leaking hidden context. `OPENAI_API_KEY` is stripped so runs ride the ChatGPT login. |
| `opencode` | `opencode run --format json -f packet.md` | File-attached packet. OpenCode defaults to edit/bash `allow`, so default review mode sets `OPENCODE_PERMISSION={"edit":"deny","bash":"deny"}` on the child env. |
...
Current preset roster (default review mode; each model+effort family on every engine that runs it):
